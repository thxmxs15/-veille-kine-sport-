...
function StarButton({ active, onClick }) {
  return (
    <button onClick={onClick} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px", fontSize: "18px", lineHeight: 1, color: active ? "#BA7517" : "var(--color-text-tertiary)", transition: "color 0.15s" }} title={active ? "Retirer des favoris" : "Ajouter aux favoris"}>
      {active ? "★" : "☆"}
    </button>
  );
}
...
function LoadingSpinner() {
  return (
    <div style={{ textAlign: "center", padding: "40px 0" }}>
      <div style={{ display: "inline-block", width: "28px", height: "28px", border: "2.5px solid var(--color-border-tertiary)", borderTopColor: "#1D9E75", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      <p style={{ fontSize: "13px", color: "var(--color-text-secondary)", marginTop: "10px" }}>Chargement des articles...</p>
    </div>
  );
}
...
function InfographicModal({ article, sources, onClose }) {
  ...
  const generateInfographic = useCallback(() => {
    ...
    ctx.fillStyle = catColor; ctx.fillRect(0, 0, W, 6);
    ctx.fillStyle = catColor + "15"; ctx.fillRect(0, 0, 6, H);

    ctx.fillStyle = catColor; ctx.fillRect(30, 40, 4, 36);
    ...
    roundRect(ctx, 46, y, ctx.measureText(cat?.label || "").width + 24, 28, 14); ctx.fill();
    ctx.font = "bold 12px 'Segoe UI', system-ui, sans-serif"; ctx.fillStyle = catColor;
    ctx.fillText(cat?.label || "", 58, y + 19); y += 52;

    ctx.strokeStyle = "#D3D1C7"; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(46, y); ctx.lineTo(W - 46, y); ctx.stroke(); y += 28;

    ctx.font = "bold 16px 'Segoe UI', system-ui, sans-serif"; ctx.fillStyle = catColor;
    ctx.fillText("Résultats clés", 46, y); y += 28;

    const findings = Array.isArray(article.key_findings) ? article.key_findings : [];
    findings.forEach((finding, i) => {
      ctx.fillStyle = catColor; ctx.beginPath(); ctx.arc(60, y - 4, 10, 0, Math.PI * 2); ctx.fill();
      ctx.font = "bold 12px 'Segoe UI', system-ui, sans-serif"; ctx.fillStyle = "#fff"; ctx.fillText(`${i + 1}`, 56, y);
    ...
    ctx.fillStyle = catColor + "10"; const boxH = 100;
    roundRect(ctx, 36, y, W - 72, boxH, 8); ctx.fill();
    ctx.strokeStyle = catColor + "30"; ctx.lineWidth = 1; roundRect(ctx, 36, y, W - 72, boxH, 8); ctx.stroke();
    ctx.font = "bold 14px 'Segoe UI', system-ui, sans-serif"; ctx.fillStyle = catColor;
    ...
    ctx.fillStyle = catColor; ctx.fillRect(0, H - 4, W, 4);
    ctx.fillText("VeilleKiné — Veille Scientifique Sport", 46, H - 18);
    setGenerated(true);
  }, [article, catColor, sources]);
  ...
  const downloadInfographic = () => {
    ...
    link.download = `infographie-${article.title.slice(0, 30)}.png`;
...
// ─── Article detail view ─────────────────────────────────────
function ArticleDetail({ article, sources, onBack, onToggleFavorite, onShowInfographic }) {
  const cat = CATEGORIES.find((c) => c.id === article.category);
  const findings = Array.isArray(article.key_findings) ? article.key_findings : [];
    ...
    <div style={{ maxWidth: "600px" }}>
      ...
      <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          ...
          <button onClick={handleAdd} disabled={adding} style={{ padding: "8px 16px", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-primary)", cursor: "pointer", fontSize: "13px", fontWeight: 500, whiteSpace: "nowrap", opacity: adding ? 0.5 : 1 }}>
            {adding ? "..." : "+ Ajouter"}
          </button>
        </div>
        <p style={{ fontSize: "11px", color: "var(--color-text-tertiary)", margin: "8px 0 0" }}>Les sources ajoutées ici sont sauvegardées dans la base — pas besoin de toucher au code</p>
      </div>
    </div>
  );
}
...
export default function VeilleApp() {
  ...
  const toggleFavorite = async (id, newValue) => {
    setArticles((prev) => prev.map((a) => (a.id === id ? { ...a, is_favorite: newValue } : a)));
    await supaUpdate("articles", id, { is_favorite: newValue });
  };

  const addSource = async (sourceData) => {
    const result = await supaInsert("sources", sourceData);
    if (Array.isArray(result) && result.length > 0) {
      setSources((prev) => [...prev, result[0]]);
    }
  };
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter((a) =>
        ...
        tags(a).some((t) => t.toLowerCase().includes(q)) ||
  ...
  if (error) return (
    <div style={{ textAlign: "center", padding: "40px 0" }}>
      <p style={{ fontSize: "14px", color: "var(--color-text-danger)" }}>{error}</p>
      <button onClick={() => window.location.reload()} style={{ marginTop: "12px", padding: "8px 20px", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-primary)", cursor: "pointer", fontSize: "13px" }}>Réessayer</button>
    </div>
  );
  ...
  if (view === "sources") {
    return (
      <div style={{ padding: "1rem 0" }}>
        <SourcesPanel sources={sources} onClose={() => setView("feed")} onAddSource={addSource} onDeleteSource={deleteSource} />
      </div>
    );
  }
  ...
  if (selectedArticle) {
    ...
    return (
      <div style={{ padding: "1rem 0" }}>
        <ArticleDetail article={art} sources={sources} onBack={() => setSelectedArticle(null)} onToggleFavorite={toggleFavorite} onShowInfographic={setInfographicArticle} />
    ...
    <div style={{ padding: "1rem 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "10px", marginBottom: "20px" }}>
        <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "12px 14px" }}>
          <p style={{ fontSize: "11px", color: "var(--color-text-secondary)", margin: "0 0 4px" }}>Total articles</p>
        ...
        <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "12px 14px" }}>
          ...
          <p style={{ fontSize: "22px", fontWeight: 500, margin: 0 }}>{sources.length}</p>
        </div>
      </div>
